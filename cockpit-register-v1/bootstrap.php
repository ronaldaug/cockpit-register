...
function registerUser($that)
{
    ...
    // --
    if (isset($data['user'])) {
        $_account = $that->storage->findOne('cockpit/accounts', ['user'  => $data['user']]);
    ...
    if (isset($data['email'])) {
        $_account = $that->storage->findOne('cockpit/accounts', ['email'  => $data['email']]);
...
function emailConfirmation($that)
{
    ...
    $id      = $decoded[0];
    $account = $that->storage->findOne('cockpit/accounts', ['_id' => $id]);
    ...
    $that->storage->save('cockpit/accounts', $account);
